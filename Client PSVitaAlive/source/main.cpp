...
int main(){
    ...
    while(screen.updateAndDraw()){
        ...
        if(catalogs.takeReady(ready,readyCatalog)){
            ...
            if(preloading){if(readyCatalog==psvitaalive::ui::CatalogType::Homebrew){screen.setCatalogItems(std::move(ready));screen.setActiveCatalog(psvitaalive::ui::CatalogType::Homebrew);homebrewReady=true;}++preloadIndex;if(preloadIndex<catalogCount){const auto next=(psvitaalive::ui::CatalogType)preloadIndex;screen.setCatalogLoading(true,psvitaalive::ui::catalogName(next),0,0,"Checking next catalog...");catalogs.request(next);}else{preloading=false;screen.setActiveCatalog(psvitaalive::ui::CatalogType::Homebrew);if(homebrewReady)screen.setCatalogLoading(false,"Homebrew",1,1,"All catalogs checked");}}else{screen.setCatalogItems(std::move(ready));screen.setActiveCatalog(readyCatalog);screen.setCatalogLoading(false,psvitaalive::ui::catalogName(readyCatalog),1,1,"Ready");}}
