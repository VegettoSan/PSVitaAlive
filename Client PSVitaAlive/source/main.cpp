...
int main(){
 ...
 psvitaalive::ui::FullCatalogScreen screen;screen.setCatalogItems(std::move(homeItems));screen.setActiveCatalog(psvitaalive::ui::CatalogType::Homebrew);screen.setImageCache(&images);
 screen.setCatalogChangeCallback([&](psvitaalive::ui::CatalogType next){if(next==psvitaalive::ui::CatalogType::Homebrew){screen.setActiveCatalog(next);return true;}screen.setActiveCatalog(next);return catalogs.request(next);});
