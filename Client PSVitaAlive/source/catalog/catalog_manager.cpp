...
bool CatalogManager::loadCatalog(ui::CatalogType catalog,std::vector<ui::CatalogItem>&outItems){
    ...
    if(haveCache){std::vector<ui::CatalogItem>cached;if(CatalogParser::parseFile(path,cached)&&!cached.empty()){/* cache first — no network on critical path (uninstall keeps ux0:data/psvitaalive) */outItems=std::move(cached);setStatus(State::Ready,catalog,"Using cached catalog");diagnostics::log(std::string("[CatalogManager] cache first (skip remote check): ")+label(catalog));http.shutdown();return true;}diagnostics::log(std::string("[CatalogManager] cache present but parse failed: ")+label(catalog));}
