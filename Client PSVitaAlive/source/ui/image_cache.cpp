...
void imageDiagnostic(const std::string&msg){SceUID fd=sceIoOpen("ux0:data/psvitaalive/logs/session.log",SCE_O_WRONLY|SCE_O_CREAT|SCE_O_APPEND,0666);if(fd<0)return;char line[1200];sceClibSnprintf(line,sizeof(line),"[ImageCache] %s\n",msg.c_str());sceIoWrite(fd,line,std::strlen(line));sceIoClose(fd);}
}
...
void ImageCache::shutdown(){stopping_=true;if(workerThread_>=0){sceKernelWaitThreadEnd(workerThread_,nullptr,nullptr);sceKernelDeleteThread(workerThread_);workerThread_=-1;}if(mutex_>=0){sceKernelDeleteMutex(mutex_);mutex_=-1;}queue_.clear();}
...
int ImageCache::workerMain(){HttpClient http;if(http.init()!=HttpResult::Ok)return-1;while(!stopping_){Job job;bool have=false;sceKernelLockMutex(mutex_,1,nullptr);if(!queue_.empty()){job=queue_.front();queue_.erase(queue_.begin());have=true;}sceKernelUnlockMutex(mutex_,1);if(!have){sceKernelDelayThread(50*1000);continue;}HttpResult r=http.downloadToFile(job.url,job.path);if(r==HttpResult::Ok&&isReady(job.path)){markReady(job.path);char m[900];sceClibSnprintf(m,sizeof(m),"ready url=%s path=%s attempt=%d",job.url.c_str(),job.path.c_str(),job.attempt+1);imageDiagnostic(m);}else{sceIoRemove(job.path.c_str());char m[1000];sceClibSnprintf(m,sizeof(m),"failed url=%s path=%s attempt=%d http=%d error=%s",job.url.c_str(),job.path.c_str(),job.attempt+1,http.lastStatusCode(),http.lastError().c_str());imageDiagnostic(m);if(job.attempt+1<MAX_RETRIES&&!stopping_){sceKernelDelayThread((job.attempt+1)*250*1000);job.attempt++;sceKernelLockMutex(mutex_,1,nullptr);queue_.push_back(job);sceKernelUnlockMutex(mutex_,1);}else markFailed(job.path);}}http.shutdown();return 0;}
} // namespace psvitaalive::ui
