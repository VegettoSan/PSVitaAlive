...
constexpr int FULL_CARD_H=120,SPLIT_CARD_H=82,DETAIL_HEADER_H=92,LINE_H=18,TRANSITION_MS=200,LINK_ROW_H=38,LINK_GAP=6,SCREENSHOT_ROW_H=250;
constexpr size_t MAX_APP_TEXTURES=9,MAX_SCREENSHOT_TEXTURES=5;constexpr uint64_t DIRECTION_REPEAT_DELAY_US=250000,DIRECTION_REPEAT_INTERVAL_US=333333;
...
void FullCatalogScreen::setCatalogItems(std::vector<CatalogItem>items){releaseTextures();allItems_=std::move(items);sortItemsByDate(allItems_);applySearch(searchQuery_);catalogLoading_=false;catalogError_.clear();}void FullCatalogScreen::setActiveCatalog(CatalogType c){releaseTextures();state_.catalog=c;searchQuery_.clear();items_=allItems_;state_.focusIndex=0;state_.catalogScrollRow=0;state_.detailScroll=0;detailScrollBeforeLinkMode_=0;state_.linkFocus=-1;state_.linkNavigation=false;}void FullCatalogScreen::setCatalogLoading(bool l,const std::string&lab,uint64_t cur,uint64_t tot,const std::string&msg){catalogLoading_=l;catalogLoadingLabel_=lab;catalogLoadingCurrent_=cur;catalogLoadingTotal_=tot;catalogLoadingMessage_=msg;if(l)catalogError_.clear();}void FullCatalogScreen::setCatalogError(const std::string&e){catalogLoading_=false;catalogError_=e;}void FullCatalogScreen::setInstallProgress(
    bool active,
    uint64_t current,
    uint64_t total,
...
bool FullCatalogScreen::init(){vita2d_init();vita2d_set_clear_color(BG);font_=vita2d_load_default_pgf();if(!font_)return false;sceCtrlSetSamplingMode(SCE_CTRL_MODE_ANALOG);state_=UiState{};ready_=true;diagnostics::log("[UI] initialized");return true;}void FullCatalogScreen::releaseTextures(){if(!textures_.empty())vita2d_wait_rendering_done();for(auto&e:textures_)if(e.second)vita2d_free_texture(e.second);textures_.clear();textureOrder_.clear();}void FullCatalogScreen::releaseScreenshotTextures(){bool any=false;for(const auto&kv:textures_)if(kv.first.find("/shot_")!=std::string::npos){any=true;break;}if(any)vita2d_wait_rendering_done();for(auto i=textures_.begin();i!=textures_.end();){if(i->first.find("/shot_")!=std::string::npos){if(i->second)vita2d_free_texture(i->second);i=textures_.erase(i);}else ++i;}textureOrder_.erase(std::remove_if(textureOrder_.begin(),textureOrder_.end(),[](const std::string&p){return p.find("/shot_")!=std::string::npos;}),textureOrder_.end());}void FullCatalogScreen::touchTexture(const std::string&p){auto i=std::find(textureOrder_.begin(),textureOrder_.end(),p);if(i!=textureOrder_.end())textureOrder_.erase(i);textureOrder_.push_back(p);}void FullCatalogScreen::evictTextureIfNeeded(const std::string&ns){if(ns=="shot")return;size_t lim=MAX_APP_TEXTURES,c=0;for(const auto&p:textureOrder_)if(p.find("/app_")!=std::string::npos)++c;if(c<lim)return;for(auto i=textureOrder_.begin();i!=textureOrder_.end();++i)if(i->find("/app_")!=std::string::npos){auto t=textures_.find(*i);if(t!=textures_.end()){if(t->second){vita2d_wait_rendering_done();vita2d_free_texture(t->second);textures_.erase(t);}}textureOrder_.erase(i);return;}}void FullCatalogScreen::shutdown(){releaseTextures();if(font_){vita2d_free_pgf(font_);font_=nullptr;}if(ready_){vita2d_fini();ready_=false;}diagnostics::log("[UI] shutdown");}
...
void FullCatalogScreen::prepareVisibleTextures(){
    ...
    auto prepareOne=[&](const std::string&url,const char*ns){
        if(loadedThisFrame||url.empty())return;
    ...
    if(state_.mode==UiMode::FULL_CATALOG){
        ...
        for(int i=first;i<last&&!loadedThisFrame;++i)prepareOne(!items_[i].icon.empty()?items_[i].icon:items_[i].cover,"app");
        return;
    }
    if(state_.mode==UiMode::OPENING_DETAIL||state_.mode==UiMode::SPLIT_DETAIL||state_.mode==UiMode::CLOSING_DETAIL){
        ...
        if(i>=0&&!loadedThisFrame){
            const CatalogItem&it=items_[i];
            const int panelX=SCREEN_W/2,panelY=HEADER_H+TABS_H,panelW=SCREEN_W-panelX,panelH=SCREEN_H-HEADER_H-TABS_H-FOOTER_H;
            const int cx=panelX+18,cw=panelW-36,top=panelY+DETAIL_HEADER_H+10,bottom=panelY+panelH-10,scroll=std::max(0,state_.detailScroll);
            int mc=std::max(18,cw/7);std::vector<std::string>pre;auto add=[&](const std::string&v){if(v.empty())return;pre.push_back("");std::vector<std::string>q;wrapText(v,mc,q);pre.insert(pre.end(),q.begin(),q.end());pre.push_back("");};add(it.description);add(it.longDescription);
            const int links=it.linkDetails.empty()?0:10+(int)it.linkDetails.size()*(LINK_ROW_H+LINK_GAP);
            const int shotTop=top+links+(int)pre.size()*LINE_H-scroll,shotH=SCREENSHOT_ROW_H-18,sc=std::min(5,(int)it.screenshots.size());
...
void FullCatalogScreen::drawCatalogCard(const CatalogItem&it,int idx,int x,int y,int w,int h,bool focus){vita2d_draw_rectangle(x,y,w,h,SURFACE);if(focus){vita2d_draw_rectangle(x,y,w,3,ACCENT);vita2d_draw_rectangle(x,y+h-3,w,3,ACCENT);vita2d_draw_rectangle(x,y,3,h,ACCENT);vita2d_draw_rectangle(x+w-3,y,3,h,ACCENT);}else vita2d_draw_rectangle(x,y,w,1,BORDER);int is=h>=100?76:54;drawImage(!it.icon.empty()?it.icon:it.cover,"app",x+10,y+9,is,is);int tx=x+is+20;vita2d_pgf_draw_text(font_,tx,y+25,WHITE,.76f,ellipsize(it.name,h>=100?24:22).c_str());vita2d_pgf_draw_text(font_,tx,y+45,TEXT,.64f,ellipsize(it.author.empty()?"Unknown author":it.author,20).c_str());vita2d_pgf_draw_text(font_,tx,y+64,colorForStatus(it.status),.62f,ellipsize(it.status,16).c_str());std::string meta=(it.version.empty()?"":"v"+it.version)+(it.versionDate.empty()?"":"  "+it.versionDate);if(meta.empty())meta=it.size;if(!meta.empty())vita2d_pgf_draw_text(font_,x+10,y+h-10,DIM,.58f,ellipsize(meta,30).c_str());(void)idx;}
void FullCatalogScreen::drawCatalogPanel(int x,int y,int w,int h,bool split){vita2d_draw_rectangle(x,y,w,h,PANEL);if(!split){int vis=visibleRowsFull(),rows=totalRows(),cw=(w-GRID_PAD*2-CARD_GAP*2)/3;for(int r=0;r<vis;++r)for(int c=0;c<3;++c){int i=(state_.catalogScrollRow+r)*3+c;if(i<(int)items_.size())drawCatalogCard(items_[i],i,x+GRID_PAD+c*(cw+CARD_GAP),y+GRID_PAD+r*(FULL_CARD_H+CARD_GAP),cw,FULL_CARD_H,i==state_.focusIndex);}if(rows>vis){int tx=x+w-7,ty=y+10,th=h-20;vita2d_draw_rectangle(tx,ty,3,th,BORDER);int thumb=std::max(20,th*vis/rows),mr=std::max(1,rows-vis),yy=ty+(th-thumb)*state_.catalogScrollRow/mr;vita2d_draw_rectangle(tx,yy,3,thumb,ACCENT);}}else for(int r=0;r<visibleRowsSplit();++r){int i=state_.catalogScrollRow+r;if(i<(int)items_.size())drawCatalogCard(items_[i],i,x+GRID_PAD,y+GRID_PAD+r*(SPLIT_CARD_H+CARD_GAP),w-GRID_PAD*2,SPLIT_CARD_H,i==state_.focusIndex);}if(state_.mode==UiMode::SPLIT_DETAIL&&state_.activePanel==UiPanel::Catalog)vita2d_draw_rectangle(x,y,3,h,ACCENT);}
...
void FullCatalogScreen::drawDetailPanel(int x,int y,int w,int h){vita2d_draw_rectangle(x,y,w,h,PANEL);int i=selectedIndex();if(i<0)return;const CatalogItem&it=items_[i];if(state_.activePanel==UiPanel::Detail)vita2d_draw_rectangle(x+w-3,y,3,h,ACCENT);vita2d_draw_rectangle(x,y,w,DETAIL_HEADER_H,SURFACE);drawImage(!it.icon.empty()?it.icon:it.cover,"app",x+12,y+12,68,68);vita2d_pgf_draw_text(font_,x+92,y+29,WHITE,.82f,ellipsize(it.name,24).c_str());vita2d_pgf_draw_text(font_,x+92,y+50,TEXT,.64f,ellipsize(it.author.empty()?"Unknown author":it.author,22).c_str());std::string meta=(it.version.empty()?"":"v"+it.version)+(it.versionDate.empty()?"":"  "+it.versionDate);vita2d_pgf_draw_text(font_,x+92,y+70,colorForStatus(it.status),.60f,ellipsize(meta.empty()?it.status:meta,25).c_str());if(!it.linkDetails.empty()){int bx=x+w-142,by=y+12,bw=128,bh=28;vita2d_draw_rectangle(bx,by,bw,bh,state_.linkNavigation?ACCENT:SURFACE2);vita2d_draw_rectangle(bx,by,bw,1,ACCENT);vita2d_draw_rectangle(bx,by,1,bh,ACCENT);vita2d_draw_rectangle(bx,by+bh-1,bw,1,ACCENT);vita2d_draw_rectangle(bx+bw-1,by,1,bh,ACCENT);vita2d_pgf_draw_text(font_,bx+10,by+19,state_.linkNavigation?BG:ACCENT,.56f,state_.linkNavigation?"△ Exit link mode":"△ Select links");}drawDetailContent(it,x,y,w,h);}
