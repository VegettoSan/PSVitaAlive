...
def build(root: Path):
    ...
    for group in groups:
        ...
        download_candidates = sorted(
            [item for item in group if item.download_url],
            key=freshness_key,
            reverse=True,
        )
        ...
        if download_candidates:
            ...
            matching_external = [
                item for item in download_candidates
        ...
        if release_candidates:
            best_version = normalize_version(release_candidates[0].version)
            release_candidates = [
        ...
        if not local:
            newest_external = max(group, key=freshness_key)
