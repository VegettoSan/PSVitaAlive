...
def author_profile(author_id, name, repo_url=None):
    ...
    if repo:
        ...
        # Validation requires at least one link. Use a GitHub user search as a
        # safe placeholder for auto-created profiles that have no known repo.
        from urllib.parse import quote
        query = quote(str(display_name))
        links.append({
            "type": "Other",
            "name": "Buscar autor",
...
def resolve_author_id(name: str, authors: dict, group: list[Candidate]) -> str:
    ...
    # Treat underscore/hyphen variants of the same slug as the same author.
    def slug_key(value: str) -> str:
        return re.sub(r"[-_]+", "-", normalize_text(value)).strip("-")

    raw_slug = slug_key(raw)
    for author_id, profile in authors.items():
        if normalize_text(profile.get("name")) == normalized:
            return author_id
        if slug_key(author_id) == raw_slug or slug_key(profile.get("name", "")) == raw_slug:
            return author_id
