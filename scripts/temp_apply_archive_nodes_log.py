...
Path('Client PSVitaAlive/include/diagnostic_logger.hpp').write_text(r'''#pragma once
...
Path('Client PSVitaAlive/source/diagnostic_logger.cpp').write_text(r'''#include "diagnostic_logger.hpp"
...
hp = 'Client PSVitaAlive/source/network/http_client.cpp'
...
'''
if old not in text:
    raise SystemExit('NETWORK_TLS diagnostics section not found')
net.write_text(text.replace(old, new, 1), encoding='utf-8')
